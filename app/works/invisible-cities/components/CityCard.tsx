...
import capitalizeString from '@/app/works/invisible-cities/components/Util'
...
export default function CityCard({ cityType, cityName, className, originalType }: CityCardProps) {
  ...
  const imagePath = `/works/invisible-cities/${typeForPath}/${href}`
  const imageUrl = `${cosBase}${imagePath}`
  const linkPath = `/works/invisible-cities/${typeForPath}/${cityName}`
